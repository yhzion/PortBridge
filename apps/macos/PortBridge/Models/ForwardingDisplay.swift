...
nonisolated struct ForwardingDisplay: Equatable {
    ...
    static func active(host: String, remotePort: Int, localPort: Int, processName: String?) -> ForwardingDisplay {
        ForwardingDisplay(status: .active, host: host, remotePort: remotePort, localPort: localPort, processName: processName, errorMessage: nil)
    }

    /// 연결 시도 중(로컬 포트는 아직 후보값이라 표시하지 않음).
    static func starting(host: String, remotePort: Int, processName: String?) -> ForwardingDisplay {
        ForwardingDisplay(status: .starting, host: host, remotePort: remotePort, localPort: nil, processName: processName, errorMessage: nil)
    }
    ...
    static func error(host: String, remotePort: Int, message: String, processName: String?) -> ForwardingDisplay {
        ForwardingDisplay(status: .error, host: host, remotePort: remotePort, localPort: nil, processName: processName, errorMessage: message)
    }
    ...
    static func inactive(host: String, remotePort: Int, processName: String?) -> ForwardingDisplay {
        ForwardingDisplay(status: .inactive, host: host, remotePort: remotePort, localPort: nil, processName: processName, errorMessage: nil)
    }
